...
void xbuf::addSeg()
{
  if (_tail)
  {
    ...
    if (_tail->next == NULL)
    {
      AHTTP_LOGDEBUG(("xbuf::addSeg: error new 1"));
    }
    else
    {
  ...
  else
  {
    ...
    if (_tail == NULL)
      AHTTP_LOGDEBUG(("xbuf::addSeg: error new 2"));
  }
...
void AsyncHTTPRequest::setDebug(bool debug)
{
  if (_debug || debug)
  {
    ...
    AHTTP_LOGDEBUG3("setDebug(", debug ? "on" : "off", ") version", ASYNC_HTTP_REQUEST_RP2040W_VERSION);
  }
...
void  AsyncHTTPRequest::_onConnect(AsyncClient* client)
{
  ...
  if (!_response)
  {
    ...
    // KH, to remove
    //AHTTP_LOGWARN("_onConnect: Can't new _responser");
    ///////
...
void  AsyncHTTPRequest::_onDisconnect(AsyncClient* client)
{
  ...
  if (_readyState < readyStateOpened)
  {
    AHTTP_LOGERROR("HTTPCODE_NOT_CONNECTED");
    _HTTPcode = HTTPCODE_NOT_CONNECTED;
  }
  else if (_HTTPcode > 0 &&
  ...
  AHTTP_LOGDEBUG1("_onDisconnect: _HTTPcode =", _HTTPcode);
  AHTTP_LOGDEBUG1("_onDisconnect: _HTTPcode =", responseHTTPString());

  // KH TODO, check to remove
  SAFE_DELETE(_client)
...
void  AsyncHTTPRequest::_onData(void* Vbuf, size_t len)
{
  ...
  if (_readyState == readyStateOpened)
  {
    if ( ! _collectHeaders())
    {
      ...
      // KH, to remove
      //AHTTP_LOGWARN("_onData: headers not complete");
      ///////
